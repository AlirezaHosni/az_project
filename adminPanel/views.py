...
from adminPanel.serializer import AdvChatSer, ReservationSerializer, RateSerializer, createAdvisorSerializer, \
    ListUsersInfoSerializer, UserSerializer, RateSerializer
...
class ListUsersInfo(APIView):
    def get(self, request, *args, **kwargs):
        ...
        each_user_hours = []
        for user in users:
            data_reservation_datetime = Reservation.objects.raw(
                "SELECT id, reservation_datetime, end_session_datetime FROM login_reservation where user_id=%s",
                [user.id])
            records_result = []
...
class AdvisorInvitations(APIView):
    def get(self, request, *args, **kwargs):
        ...
        advisors_list = []
        for adv in advisors:
            data_reservation_datetime = Invitation.objects.raw(
                "SELECT id, COUNT(id) as num_of_inv FROM login_invitation where advisor_id=%s group by id",
                [adv.advisor_id])
...
class ListAdvisorChat(APIView):
    def get(self, request, *args, **kwargs):
        ...
        for user in users:
            data_reservation_datetime = Reservation.objects.raw(
                "SELECT id, reservation_datetime, end_session_datetime FROM login_reservation where advisor_user_id=%s",
                [user.id])
            records_result = []
            sum_of_serssion_hours = 0
            for rec in data_reservation_datetime:
...
class RetrieveParticularAdvisorChats(APIView):
    def get(self, request, *args, **kwargs):
        ...
        objs = []
        for chat in chats:
            contact = User.objects.raw(
                "select * from login_user where id in (select user_id from chat_chat_user where chat_id=%s and user_id !=%s)",
...
class ListReservationDetails(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        result_list = []
        for r in res:
            advisor = User.objects.raw(
                "select u.id, image, first_name, last_name from login_advisor as a inner join login_user as u on a.user_id=u.id where u.id=%s",
                [r.advisor_user_id])
    ...
    def get(self, request, *args, **kwargs):
        ...
        for i in users:
            ...
            user = User.objects.raw(
                "select u.id, u.id as user_id, a.id as advisor_id, first_name, last_name from login_user as u inner join login_advisor as a on u.id=a.user_id where a.id=%s",
                [i.advisor_id])
            us.append({
                "id": user[0].id,
