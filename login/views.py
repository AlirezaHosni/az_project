...
from .serializer import RateFinderSerializer, AdvisorInfoSerializer, professionFinder, AdvisorResumeSerializer, \
    ListRateSerializer, RateSerializer, CreateRequestSerializer, RequestUpdateSerializer, RequestSerializer, \
    SearchInfoSerializer, RegisterSerializer, UserSerializer, AdvisorSerializer
...
class AdvisorRequestsInfoAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Request.objects.raw(
            'select email,first_name,last_name,gender,image,r.id,request_content,is_checked,is_blocked,is_accepted,created_at,is_Done from login_user as u inner join login_request as r on u.id=r.sender_id where r.receiver_id in (select a.id from login_advisor as a inner join login_user as u on a.user_id=u.id where u.id=%s) and is_checked=false',
            [self.request.user.id])
...
class ListRateByAdvisorIdAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Rate.objects.raw(
            'select rate.id, rate.text, rate.rate, rate.created_at , user.first_name, user.last_name, user.image  from login_rate as rate inner join login_user as user on rate.user_id=user.id where rate.advisor_id in (select a.id from login_advisor as a inner join login_user as u on a.user_id=u.id where a.id=%s)',
            [advisor_id])
